...
// FetchMergeQueue is deprecated - use FetchInternalMRs instead.
// Returns empty slice for backward compatibility.
func (f *LiveConvoyFetcher) FetchMergeQueue() ([]MergeQueueRow, error) {
	return nil, nil
}

// gitURLToRepoPath converts a git URL to owner/repo format for GitHub URLs.
...
func (f *LiveConvoyFetcher) FetchPolecats() ([]PolecatRow, error) {
	...
	// Pre-fetch internal MR count to determine refinery idle status
	mrCount := f.getInternalMRCount()
	...
	for _, line := range lines {
		...
		if polecat == "refinery" {
			statusHint = f.getRefineryStatusHint(mrCount)
		} else {
...
// getInternalMRCount returns the total number of open internal MRs.
func (f *LiveConvoyFetcher) getInternalMRCount() int {
	mrs, err := f.FetchInternalMRs()
	if err != nil {
		return 0
	}
	return len(mrs)
...
// getRefineryStatusHint returns appropriate status for refinery based on internal MRs.
func (f *LiveConvoyFetcher) getRefineryStatusHint(mrCount int) string {
	if mrCount == 0 {
	...
	return fmt.Sprintf("Processing %d MRs", mrCount)
}
